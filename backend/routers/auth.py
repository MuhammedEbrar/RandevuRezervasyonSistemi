...
from backend.schemas.user import UserCreate, UserOut, UserLogin # Kullanıcı şemalarınız
from backend.crud import user as crud_user # Stajyer 1'in yazdığı CRUD fonksiyonları
from backend.core.security import get_password_hash, verify_password, create_access_token # Güvenlik fonksiyonları
from backend.models.user import User, UserRole # SQLAlchemy User modeli
from backend.database import get_db # Veritabanı oturumu için bağımlılık
...
async def login_for_access_token(
    ...
    access_token_data = {
        "sub": str(user.id), # user.id bir UUID ise str'ye çevirin
        "email": user.email,
