...
from models import (
    User, Resource, Booking, BookingStatus, DayOfWeek,
    ScheduleType, BookingType, AvailabilitySchedule
)

# Router'ın tanımlanması
...
async def create_availability_schedule(
    schedule_in: AvailabilityScheduleCreate,   # DÜZELTME: Veriyi tek bir Pydantic objesi olarak body'den alıyoruz.
...
async def get_resource_availability_schedules(
    ...
    """Bir kaynağın sahibi tarafından tanımlanmış tüm müsaitlik kurallarını listeler."""
    check_resource_ownership(db, current_user, resource_id)
