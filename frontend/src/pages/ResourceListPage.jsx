...
function ResourceListPage() {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error('Varlıkları çekerken bir sorun oluştu.');
  ...
  const handleDelete = async (resourceIdToDelete) => {
    if (!window.confirm('Bu varlığı silmek istediğinize emin misiniz? Bu işlem geri alınamaz.')) {
      return;
    }
    const token = localStorage.getItem('userToken');
    const apiUrl = `${import.meta.env.VITE_API_BASE_URL}/resources/${resourceIdToDelete}`;
    try {
      const response = await fetch(apiUrl, {
        method: 'DELETE',
        headers: {
      ...
      setResources(resources.filter(resource => resource.resource_id !== resourceIdToDelete));
    ...
    <div className="container mx-auto p-8">
      ...
      {resources.length === 0 ? (
        <div className="text-center py-10 bg-white rounded-lg shadow-md">
            <p className="text-gray-500">Henüz bir varlık eklemediniz.</p>
        </div>
      ) : (
            ...
            {resources.map(resource => (
              <li key={resource.resource_id} className="p-4 flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 hover:bg-gray-50">
                {resource.images && resource.images.length > 0 ? (
                  <img src={resource.images[0]} alt={resource.name} className="w-24 h-24 object-cover rounded-md flex-shrink-0" />
                ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:space-x-2 space-y-2 sm:space-y-0">
                    <Link to={`/dashboard/resources/edit/${resource.resource_id}`} className="text-sm bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-3 rounded text-center">Düzenle</Link>
